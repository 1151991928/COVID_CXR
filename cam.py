...
model.load_state_dict(torch.load('pth/model_epoch_19.pth'))

img_path='dataset\CXR\images\P_1_6.jpeg'
data_transforms = transforms.Compose([
    transforms.Resize(256),    # 将图片短边缩放至256，长宽比保持不变：
    transforms.CenterCrop(224),   #将图片从中心切剪成3*224*224大小的图片
    transforms.ToTensor()          #把图片进行归一化，并把数据转换成Tensor类型
])
draw_CAM(model, img_path, 'cam.jpg', data_transforms, visual_heatmap=True)